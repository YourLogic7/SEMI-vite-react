...
const loadState = () => {
    try {
        ...
        if (serializedState === null) {
            return undefined; // Gunakan initial state jika tidak ada data
        }
        // Pastikan untuk mengurai objek JSON dengan benar, termasuk data angka
        const savedState = JSON.parse(serializedState);

        // Memeriksa dan mengonversi data numerik yang mungkin disimpan sebagai string
...
const appReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'ADD_USER': {
            ...
            // Hanya tambahkan jika pengguna belum ada
...
// Coba memuat state yang tersimpan, atau gunakan initialState jika tidak ada
...
export default function App() {
    ...
    // useEffect ini akan menyimpan state ke localStorage setiap kali ada perubahan
    useEffect(() => {
        // Hanya simpan state yang relevan dan bisa di-serialisasi
        const stateToSave = {
            user: state.user,
            users: state.users,
    ...
    const {
        ...
        notifications, isSuperAdmin, visibleProductsCount, checkoutTotal, wishlist,
    ...
    const getSellerName = (sellerId) => sellers.find(s => s.id === sellerId)?.name || 'Penjual Tidak Dikenal';
    ...
    const filteredProducts = useMemo(() => {
        let prods = processedProducts.filter(p => !flashSaleProducts.some(fp => fp.id === p.id));
    ...
    const handleRegister = (name, email) => {
        const newUser = { id: Date.now(), name, email, role: 'customer', displayName: name };
        dispatch({ type: 'SET_USER', payload: newUser });
        dispatch({ type: 'ADD_USER', payload: newUser }); // Tambahkan user baru ke daftar user
    ...
    const mainContentProps = {
        ...
        flashSaleProducts: processedProducts.filter(p => flashSaleProducts.some(fp => fp.id === p.id)),
        sellers,
    ...
    const renderPage = () => {
        switch (currentView.page) {
            ...
            />;
            case 'storePage': return <StorePage seller={currentView.data} products={products.filter(p => p.sellerId === currentView.data.id)} onNavigate={(page, data) => dispatch({ type: 'NAVIGATE', payload: { page, data } })} sellers={sellers} addToCart={addToCart} handleBuyNow={handleBuyNow} />;
